      ...
      >
        {pageDevices.map((device) => (
          <div key={device.serial} style={cardStyle}>
            <DeviceCard
              device={device}
              screenshot={screenshots.get(device.serial)}
              selected={selectedSerials.has(device.serial)}
              onSelect={(selected) => {
                if (selected) {
                  selectedSerials.add(device.serial);
                } else {
                  selectedSerials.delete(device.serial);
                }
              }}
              onClickDevice={cmds.clickDevice}
              onInputText={cmds.inputText}
              onKeyEvent={cmds.keyEvent}
              onSwipe={cmds.swipeDevice}
              onWakeUp={cmds.wakeUpDevice}
            />
          </div>
        ))}
      </div>

      {/* Pagination or Overview Mode Toggle */}
